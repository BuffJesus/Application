...
{
  internal static \u0024ArrayType\u0024\u0024\u0024BY06\u0024\u0024CBH FableMod\u002ELEV\u002E\u003FA0xe83841bc\u002ECELL_SIZES;
  internal static \u0024ArrayType\u0024\u0024\u0024BY06\u0024\u0024CBM FableMod\u002ELEV\u002E\u003FA0xe83841bc\u002ESIZES;
  internal static int __\u0040\u0040_PchSym_\u004000\u0040UuzyovnlwUhixUuzyovnlwOoveUcGEUivovzhvUhgwzucOlyq\u0040;
  ...
  internal static bool FableMod\u002ELEV\u002E\u003FA0xe83841bc\u002EHitTestX(
    float x1,
    float x2,
    float x3,
    float x4)
  {
    return (double) x3 <= (double) x1 && (double) x3 >= (double) x2 && (double) x4 <= (double) x1 && (double) x4 >= (double) x2 || (double) x1 <= (double) x3 && (double) x1 >= (double) x4 && (double) x2 <= (double) x3 && (double) x2 >= (double) x4;
  }

  [return: MarshalAs(UnmanagedType.U1)]
  internal static bool FableMod\u002ELEV\u002E\u003FA0xe83841bc\u002EHitTestY(
    float y1,
    float y2,
    float y3,
    float y4)
  {
    return (double) y3 >= (double) y1 && (double) y3 <= (double) y2 && (double) y4 >= (double) y1 && (double) y4 <= (double) y2 || (double) y1 >= (double) y3 && (double) y1 <= (double) y4 && (double) y2 >= (double) y3 && (double) y2 <= (double) y4;
  }

  internal static unsafe NavNode FableMod\u002ELEV\u002E\u003FA0xe3ba95e2\u002EReadNode(
    BufferStream fileIn)
  {
    \u0024ArrayType\u0024\u0024\u0024BY02E arrayTypeBy02E;
    fileIn.Read((void*) &arrayTypeBy02E, 3);
    ...
    // ISSUE: cast to a reference type
    // ISSUE: explicit reference operation
    if (^(byte&) ref arrayTypeBy02E == (byte) 0 && ^(byte&) ((IntPtr) &arrayTypeBy02E + 1) == (byte) 1 && ^(byte&) ((IntPtr) &arrayTypeBy02E + 2) == (byte) 1)
      return new NavNode() { Type = NavNodeType.Blank };
    byte num;
    fileIn.Read((void*) &num, 1);
    NavNode navNode;
    // ISSUE: cast to a reference type
    // ISSUE: explicit reference operation
    if (^(byte&) ref arrayTypeBy02E == (byte) 0)
    {
      if (num == (byte) 0)
        navNode = (NavNode) new StandardNode();
      else if (num == (byte) 1)
        navNode = (NavNode) new NavigationNode();
      else
        goto label_12;
    }
    else
    {
      // ISSUE: cast to a reference type
      // ISSUE: explicit reference operation
      if (^(byte&) ref arrayTypeBy02E == (byte) 1 && num == (byte) 1)
        navNode = (NavNode) new DynamicNode();
      else
        goto label_12;
    }
    // ISSUE: cast to a reference type
    // ISSUE: explicit reference operation
    Debug.Assert(^(byte&) ((IntPtr) &arrayTypeBy02E + 1) <= (byte) 1);
    if (navNode == null)
      return (NavNode) null;
    navNode.Load(fileIn);
    return navNode;
label_12:
    throw new InvalidOperationException("FABLEMOD_EXCEPTION_INVALID_NAVIGATION_NODE");
  }
